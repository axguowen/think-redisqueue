...
class Command extends Base
{
    ...
    {
        // 指令配置
        $this->setName('redisqueue')
            ->addArgument('action', Argument::OPTIONAL, 'start|stop|restart|reload|status', 'start')
            ->addOption('daemon', 'd', Option::VALUE_NONE, 'Run the redisqueue in daemon mode.')
            ->setDescription('A Simple Redis Queue Extend For ThinkPHP');
    }
    ...
    protected function execute(Input $input, Output $output)
    {
        ...
        // 如果是启动
        if ('start' == $action) {
            $output->writeln('Starting redisqueue service...');
        }

        // 实例化
        $redisqueue = $this->app->make(RedisQueue::class);
